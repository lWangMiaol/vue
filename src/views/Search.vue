<template>
  <div class="search">
    ...
    <!-- <div v-show="!searchVal" class="distance">
      <ul>
        <li v-for="data in getCinemaListFive" :key="data.cinemaId">{{data.name}}</li>
      </ul>
    </div>-->
...
import Vue from "vue";
import { Srarch, Search, List, Cell, Empty } from "vant";
...
export default {
  ...
  computed: {
    ...
    searchCinemaData() {
      let data = this.cinemaList;
      let searchCon = data.filter(item => {
        return (
